...
const transactionController = {
  async create(req, res, next) {
    try {
      const { items, customerEmail, customerName, paymentMethod, notes, discount, tax } = req.body;
      if (!items || !Array.isArray(items) || items.length === 0) {
        return res.status(400).json({ success: false, message: 'Items are required' });
      }
      const transaction = await transactionService.create(req.tenantId, req.user.userId, {
        items, customerEmail, customerName, paymentMethod, notes, discount: +discount || 0, tax: +tax || 0,
      });
